...
const publishEvent = async (routingKey, payload) => {
  try {
    ...
    if (published) {
      logger.info('Event published', { routingKey, loanId: payload.loanId });
    } else {
...
const publishLoanCreated = async (loan) => {
  return publishEvent('loan.created', {
    loanId: loan.id,
    bookId: loan.bookId,
    userId: loan.userId,
...
const publishLoanReturned = async (loan) => {
  return publishEvent('loan.returned', {
    loanId: loan.id,
    bookId: loan.bookId,
    userId: loan.userId
  });
